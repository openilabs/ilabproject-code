...
{
    public class DriverVoltageVsSpeed : DriverGeneric
    {
        ...
        public DriverVoltageVsSpeed(int unitId)
            : this(unitId, null)
        {
        }
        ...
        public DriverVoltageVsSpeed(int unitId, CancelExperiment cancelExperiment)
        ...
        public override int GetExecutionTime(ExperimentSpecification experimentSpecification)
        {
            ...
            executionTime += this.redLion.StartDCDriveMutExecutionTime(RedLion.StartDCDriveMutModes.EnableOnly);

            //
            // Determine measurement time
            //
            int vectorLength = ((specification.Speed.max - specification.Speed.min) / specification.Speed.step) + 1;
            int measurementsTime = this.redLion.SetSpeedACDriveExecutionTime();
            measurementsTime += this.redLion.MeasurementCount * this.redLion.MeasurementDelay;
            executionTime += (vectorLength * measurementsTime) + this.redLion.SetSpeedACDriveExecutionTime();
                ...
                switch (state)
                {
                    ...
                    case States.sConfigureACDrive:
                        ...
                        RedLion.ACDriveConfiguration setACConfig = new RedLion.ACDriveConfiguration(
                            specification.Speed.min, 0, ACDrive.DEFAULT_SpeedRampTime, ACDrive.DEFAULT_MaximumCurrent,
                    ...
                    case States.sStartDCDriveMut:
                        ...
                        //
                        // Enable DC drive but don't start it
                        //
                        if (this.redLion.StartDCDriveMut(RedLion.StartDCDriveMutModes.EnableOnly) == true)
                        {
                    ...
                    case States.sTakeMeasurements:
                        ...
                        if (vectorIndex == vectorLength)
                        {
                            ...
                            if (this.redLion.SetSpeedACDrive(0) == true)
                            {
                            }
                        ...
                        //
                        // Set the AC drive speed
                        //
                        int speed = specification.Speed.min + (vectorIndex * specification.Speed.step);
                        if (this.redLion.SetSpeedACDrive(speed) == true)
                        {
                            measurementNumber = 0;
