...
{
    public class DriverVoltageVsLoad : DriverGeneric
    {
        ...
        public DriverVoltageVsLoad(int unitId)
            : this(unitId, null)
        {
        }
        ...
        public DriverVoltageVsLoad(int unitId, CancelExperiment cancelExperiment)
        ...
        public override int GetExecutionTime(ExperimentSpecification experimentSpecification)
        {
            ...
            executionTime += this.redLion.StartDCDriveMutExecutionTime(RedLion.StartDCDriveMutModes.Torque);

            //
            // Determine measurement time
            //
            int vectorLength = ((specification.Load.max - specification.Load.min) / specification.Load.step) + 1;
            int measurementsTime = this.redLion.SetTorqueDCDriveMutExecutionTime();
            measurementsTime += this.redLion.MeasurementCount * this.redLion.MeasurementDelay;
            executionTime += (vectorLength * measurementsTime) + this.redLion.SetTorqueDCDriveMutExecutionTime();
                ...
                switch (state)
                {
                    ...
                    case States.sConfigureACDrive:
                        ...
                        RedLion.ACDriveConfiguration setACConfig = new RedLion.ACDriveConfiguration(
                            ACDrive.MAXIMUM_Speed, 0, ACDrive.DEFAULT_SpeedRampTime, ACDrive.MAXIMUM_MaximumCurrent,
                    ...
                    case States.sStartDCDriveMut:
                        ...
                        // Start DC drive
                        //
                        if (this.redLion.StartDCDriveMut(RedLion.StartDCDriveMutModes.Torque) == true)
                        {
                    ...
                    case States.sTakeMeasurements:
                        ...
                        if (vectorIndex == vectorLength)
                        {
                            ...
                            if (this.redLion.SetTorqueDCDriveMut(0) == false)
                            {
                                resultInfo.statusCode = StatusCodes.Failed;
                                resultInfo.errorMessage = this.redLion.LastError;
                            }
                        ...
                        //
                        // Set the torque percentage
                        //
                        int percent = specification.Load.min + (vectorIndex * specification.Load.step);
                        if (this.redLion.SetTorqueDCDriveMut(percent) == true)
                        {
                            measurementNumber = 0;
