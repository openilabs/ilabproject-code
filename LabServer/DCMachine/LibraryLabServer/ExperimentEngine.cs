...
namespace Library.LabServer
{
    public class ExperimentEngine : LabExperimentEngine
    {
        ...
        public override ValidationReport Validate(string xmlSpecification)
        {
            ...
            try
            {
                Specification specification = new Specification(this.configuration);
        ...
        public override ExperimentInfo RunExperiment(ExperimentInfo experimentInfo)
        {
            ...
            try
            {
                ...
                {
                    driver = new DriverVoltageVsSpeed(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_VoltageVsField))
                {
                    driver = new DriverVoltageVsField(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_VoltageVsLoad))
                {
                    driver = new DriverVoltageVsLoad(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_SpeedVsVoltage))
                {
                    driver = new DriverSpeedVsVoltage(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_SpeedVsField))
                {
                    driver = new DriverSpeedVsField(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }

                //
                // Execute the experiment and return the result information
                //
                ResultInfo resultInfo = (ResultInfo)driver.Execute(specification);
