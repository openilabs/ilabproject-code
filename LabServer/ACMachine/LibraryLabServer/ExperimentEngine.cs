    ...
    public class ExperimentEngine : LabExperimentEngine
    {
        ...
        public override ExperimentInfo RunExperiment(ExperimentInfo experimentInfo)
        {
            ...
            try
            {
                ...
                {
                    driver = new DriverLockedRotor(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_NoLoad))
                {
                    driver = new DriverNoLoad(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_SynchronousSpeed))
                {
                    driver = new DriverSynchronousSpeed(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
                else if (specification.SetupId.Equals(Consts.STRXML_SetupId_FullLoad))
                {
                    driver = new DriverFullLoad(experimentInfo.unitId, this.labExperimentInfo.cancelExperiment);
                }
