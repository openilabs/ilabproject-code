...
{
	/// <summary>
    /// Summary description for addEditServAdminGroupPopup.
	/// </summary>
    public partial class addEditServAdminGroupPopup : System.Web.UI.Page
    {
        ...
        /// The parent list is restricted to groups that are service administration groups
        /// </summary>
        private void loadParentLists()
        {
            ...
            foreach (Group grp in allGroups)
            {
                if ((grp.groupID > 0) && (grp.groupID != groupInfo.id))
                {
                    ...
                    if (notAssociated)
                    {
                        // Note: group should be a service admin group to be added to the parent list
                        if ((grp.groupID > 0) 
                            && (!grp.groupType.Equals(GroupType.REQUEST))
        ...
        protected void saveNew()
        {
            ...
            try
            {
                ...
                groupInfo.id = wrapper.AddGroupWrapper(txtName.Text, tmpParent, txtDescription.Text, txtEmail.Text, GroupType.SERVICE_ADMIN, 0); //AddGroup
