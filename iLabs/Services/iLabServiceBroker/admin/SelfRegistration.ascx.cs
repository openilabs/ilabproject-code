...
namespace iLabs.WebService
{
    ...
    public partial class SelfRegistration : System.Web.UI.UserControl
	{
        ...
        private string guidMessage = "A ServiceBroker Guid must be unique and for it to work with Batch Lab Servers must be no longer than 35 character";
 ...
 /*       public event EventHandler Modify = this.modifyService;
        public event EventHandler Refresh = this.btnRefresh_Click;
        public event EventHandler Clear = this.btnNew_Click;
        public event EventHandler Save = this.saveChanges;
        public event EventHandler Retire = this.btnRetire_Click;
  * */
        ...
        {
            if(args.Value.Trim().Length >0 && args.Value.Trim().Length <=35)
                args.IsValid = true;
            else 
                args.IsValid = false;
        }

        public void modifyServiceSB(object sender, System.EventArgs e)
        {
            bool error = false;
            StringBuilder message = new StringBuilder();
            try
            {
                if (ProcessAgentDB.ServiceAgent != null)
                {
                    ...
                    if (ProcessAgentDB.ServiceAgent.domainGuid != null)
                    {
                        ...
                        int returnValue = dbTicketing.ModifyProcessAgent(originalGuid, pa, null);
                ...
                {
                    dbTicketing.SelfRegisterProcessAgent(ProcessAgentDB.ServiceAgent.agentGuid.Trim(), txtServiceName.Text.Trim(), agentType,
                            null, txtCodebaseUrl.Text.Trim(), txtWebServiceUrl.Text.Trim());
                    bakServiceName.Value = txtServiceName.Text.Trim();
                    bakCodebase.Value = txtCodebaseUrl.Text.Trim();
                    bakServiceUrl.Value = txtWebServiceUrl.Text.Trim();
                }
            }
            catch (Exception ex)
            {
