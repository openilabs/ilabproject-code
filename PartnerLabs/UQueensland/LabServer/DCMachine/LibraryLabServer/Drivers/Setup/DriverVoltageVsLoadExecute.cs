...
namespace Library.LabServer.Drivers.Setup
{
    public partial class DriverVoltageVsLoad : DriverEquipmentGeneric
    {
        ...
        private enum States_Execute
        {
            ...
            sStartACDrive,
            sStartDCDriveMut,
            sSetTorqueDCDriveMut,
            sTakeMeasurement,
            sResetTorqueDCDriveMut,
            sStopDCDriveMut,
            sStopACDrive,
        ...
        private SMTableEntry_Execute[] smTable_Execute = new SMTableEntry_Execute[] {
            ...
            // CreateConnection
            //
            new SMTableEntry_Execute(States_Execute.sCreateConnection, States_Execute.sGetACDriveInfo, States_Execute.sResumePowerdown,
                Consts.STRXML_CmdCreateConnection, null),
            //
            // GetACDriveInfo
            //
            new SMTableEntry_Execute(States_Execute.sGetACDriveInfo, States_Execute.sGetDCDriveMutInfo, States_Execute.sCloseConnection,
                Consts.STRXML_CmdGetACDriveInfo, null),
            //
            // GetDCDriveMutInfo
            //
            new SMTableEntry_Execute(States_Execute.sGetDCDriveMutInfo, States_Execute.sResetACDrive, States_Execute.sCloseConnection,
                Consts.STRXML_CmdGetDCDriveMutInfo, null),
            //
            // ResetACDrive
            //
            ...
            // ConfigureACDrive
            //
            new SMTableEntry_Execute(States_Execute.sConfigureACDrive, States_Execute.sSetSpeedACDrive, States_Execute.sReconfigureACDrive,
            ...
            // SetSpeedACDrive
            //
            new SMTableEntry_Execute(States_Execute.sSetSpeedACDrive, States_Execute.sConfigureDCDriveMut, States_Execute.sReconfigureACDrive,
                Consts.STRXML_CmdSetSpeedACDrive, new string[,] {
                    { Consts.STRXML_ReqSpeedACDrive, string.Empty }
                } ),
            //
            // ConfigureDCDriveMut
            //
            ...
            // StartDCDriveMut
            //
            new SMTableEntry_Execute(States_Execute.sStartDCDriveMut, States_Execute.sSetTorqueDCDriveMut, States_Execute.sStopDCDriveMut,
                Consts.STRXML_CmdStartDCDriveMut, new string[,] {
                    { Consts.STRXML_ReqDCDriveMutMode, Consts.STR_DCDriveMutMode_Torque }
                } ),
            //
            // SetTorqueDCDriveMut
            //
            new SMTableEntry_Execute(States_Execute.sSetTorqueDCDriveMut, States_Execute.sTakeMeasurement, States_Execute.sStopDCDriveMut,
                Consts.STRXML_CmdSetTorqueDCDriveMut, new string[,] {
                    { Consts.STRXML_ReqTorqueDCDriveMut, string.Empty }
                } ),
            //
            // TakeMeasurement
            //
            new SMTableEntry_Execute(States_Execute.sTakeMeasurement, States_Execute.sResetTorqueDCDriveMut, States_Execute.sStopDCDriveMut,
                Consts.STRXML_CmdTakeMeasurement, null),
            //
            // ResetTorqueDCDriveMut
            //
            new SMTableEntry_Execute(States_Execute.sResetTorqueDCDriveMut, States_Execute.sStopDCDriveMut, States_Execute.sStopDCDriveMut,
                Consts.STRXML_CmdSetTorqueDCDriveMut, new string[,] {
                    { Consts.STRXML_ReqTorqueDCDriveMut, string.Empty }
                } ),
            //
        ...
        public override ExperimentResultInfo Execute(ExperimentSpecification experimentSpecification)
        {
            ...
            //
            int vectorLength = ((specification.Load.max - specification.Load.min) / specification.Load.step) + 1;
            ...
            //
            int maxSpeedACDrive = 0;
            int defaultTorqueDCDriveMut = 0;
            int vectorIndex = 0;
            int repeatCount = 0;
                ...
                while (state != States_Execute.sCompleted)
                {
                    ...
                    switch (entry.currentState)
                    {
                        case States_Execute.sSetSpeedACDrive:
                            int speedACDrive = maxSpeedACDrive;
                            entry.commandArguments[0, 1] = speedACDrive.ToString();
                            break;

                        case States_Execute.sSetTorqueDCDriveMut:
                            int torqueDCDriveMut = specification.Load.min + (vectorIndex * specification.Load.step);
                            entry.commandArguments[0, 1] = torqueDCDriveMut.ToString();
                            break;

                        case States_Execute.sResetTorqueDCDriveMut:
                            torqueDCDriveMut = defaultTorqueDCDriveMut;
                            entry.commandArguments[0, 1] = torqueDCDriveMut.ToString();
                            break;
                    ...
                    switch (entry.currentState)
                    {
                        case States_Execute.sGetACDriveInfo:
                            maxSpeedACDrive = XmlUtilities.GetIntValue(xmlResponseNode, Consts.STRXML_RspMaxSpeed, 0);
                            break;

                        case States_Execute.sGetDCDriveMutInfo:
                            defaultTorqueDCDriveMut = XmlUtilities.GetIntValue(xmlResponseNode, Consts.STRXML_RspDefaultTorque, 0);
                            break;
                        ...
                        case States_Execute.sTakeMeasurement:
                            ...
                            if (++repeatCount == this.measurementCount)
                            {
                                ...
                                //
                                repeatCount = 0;
                                nextState = States_Execute.sSetTorqueDCDriveMut;
                                break;
                            }
