...
namespace Library.LabServer.Drivers.Module
{
    ...
    public class NTPClient
    {
        ...
        private class UdpState
        {
            ...
            public UdpState(IPEndPoint endPoint, UdpClient udpClient, ref Byte[] receiveData)
            {
        ...
        public static void ReceiveCallback(IAsyncResult asyncResult)
        {
            ...
            Byte[] receiveData = (Byte[])((UdpState)(asyncResult.AsyncState)).receiveData;

            Byte[] receiveBytes = udpClient.EndReceive(asyncResult, ref endPoint);
            receiveBytes.CopyTo(receiveData, 0);
        }
		...
		{
			if (NTPData.Length < NTPDataLength)
            {
                return false;
            }
            else if (Mode != _Mode.Server)
			{
				return false;
			}
			else
