...
Imports WebLab.Elvis
Imports ILabStandardTypes

Namespace WebLab.LabServer
    ...
    Public Class LabServerAPI : Inherits WebService
        ...
        Public Function GetLabStatus() As LabStatus
            ...
            'attempt to read sbAuthHeader object from SOAP header
        ...
        Public Function GetEffectiveQueueLength(ByVal userGroup As String, ByVal priorityHint As Integer) As WaitEstimate
            ...
            'attempt to read BrokerCred object from SOAP header
            If Not BrokerCred Is Nothing Then
                Try
                    intBrokerID = AuthenticateCaller(BrokerCred.identifier, BrokerCred.passKey)
                    BrokerCred.DidUnderstand = True
        ...
        <WebMethod(Description:="WebLab Lab Server Method: Returns the valid XML lab configuration document 'labConfiguration'.  Used by the lab client to determine experiment setup availability."), SoapHeader("BrokerCred", Direction:=SoapHeaderDirection.In), SoapDocumentMethod(RequestNamespace:="http://ilab.mit.edu")> _
        Public Function GetLabConfiguration(ByVal userGroup As String) As String
            'This method returns a valid XML lab configuration document.  This document describes the setups available on the lab server for the 
            'specified Group.  
            Dim rpmObject As ResourcePermissionManager = New ResourcePermissionManager
            Dim rmObject As RecordManager = New RecordManager
            ...
            Dim blnHasPermission As Boolean = False
            Debug.WriteLine("The lab configuration method has been called with usergroup " & userGroup)

            'attempt to read BrokerCred object from SOAP header
            If Not BrokerCred Is Nothing Then
                Debug.WriteLine("Broker cred has been initialized properly with ID " & BrokerCred.identifier & "  and passKey " & BrokerCred.passKey)
                Try
                    intBrokerID = AuthenticateCaller(BrokerCred.identifier, BrokerCred.passKey)
                ...
                Debug.WriteLine("The resulting broker ID is " & intBrokerID)
                'once identity is determined, check caller is authorized to use this interface
                blnHasPermission = AuthorizeBroker(intBrokerID, "canview")
                ...
                If blnHasPermission Then 'caller has sufficient permission
                    ...
                    If GetInterfaceStatus() Then 'interface is enabled
                        Try
                            ...
                            If intGroupID > 0 Then
                                'use group id
                                strValReport = valEngine.validate(experimentSpecification, intBrokerID, intGroupID)
                            Else
                                'use broker id
                                strValReport = valEngine.validate(experimentSpecification, intBrokerID)
                            End If
                            ...
                            Else
                                ...
                                vrOutput.estRuntime = CDbl(rmObject.ExperimentRuntimeEst(valEngine.getTotalDataPoints(), valEngine.getSetupID()))
                            End If
                        ...
                        If intExpIDCheck = -1 Then
                            Try
                                ...
                                If blnUseGroup Then
                                    ...
                                    intPriority = rpmObject.GetGroupSetupPriority(intGroupID, valEngine.getResourceID())
                                    ...
                                    intEstRunTime = rmObject.ExperimentRuntimeEst(valEngine.getTotalDataPoints(), valEngine.getSetupID())
                                    ...
                                    'submit job
                                    Dim intSubmitReturn As Integer = qmObject.EnqueueJob(experimentSpecification, intPriority, userGroup, intBrokerID, experimentID, intEstRunTime, valEngine.getTotalDataPoints(), valEngine.getSetupID())

                                    srOutput.minTimetoLive = CDbl(12)       'may want to make this a admin-definable setting
                                    weOutput.effectiveQueueLength = intSubmitReturn
                                ...
                                Else
                                    ...
                                    'priority determination
                                    intPriority = rpmObject.GetBrokerSetupPriority(intBrokerID, valEngine.getResourceID())
                                    ...
                                    weOutput.estWait = queueLength.EstTimeToRun()
                                    'submit job
                                    Dim intSubmitReturn As Integer = qmObject.EnqueueJob(experimentSpecification, intPriority, "", intBrokerID, experimentID, intEstRunTime, valEngine.getTotalDataPoints(), valEngine.getSetupID())
                ...
                If blnHasPermission Then 'caller has sufficient permission
                    ...
                    If GetInterfaceStatus() Then 'if interface is active
                        Try
                            ...
                            esOutput.estRemainingRuntime = expStatus.EstExecTime()
                            esOutput.wait = weOutput
                            lesOutput.minTimetoLive = 12
                            lesOutput.statusReport = esOutput
                        Catch
                ...
                If blnHasPermission Then 'caller has sufficient permission
                    ...
                    If GetInterfaceStatus() Then 'if interface is active
                        Try
                            ...
                            rrOutput.labConfiguration = resultObject.LabConfig()
                            ...
                            If eriObject.SubmitTime() <> "" Then 'checks submit time field for data
                                ...
                                strResultExtension = strResultExtension & ",SetupName=" & eriObject.SetupName()
                        ...
                        Catch
                            'error case
                            rmObject.LogIncomingWebRequest(intBrokerID, "RetrieveResult", blnHasPermission, False, "Method Error: " & Err.Description() & ": " & Err.Source())
                            Throw New Protocols.SoapException("Method Error: " & Err.Description() & ": " & Err.Source(), Protocols.SoapException.ServerFaultCode)
                            'Return rrOutput
                            Exit Function
