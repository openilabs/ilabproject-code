...
<script Runat="Server">
	...
	Dim strPageState, strErrorMsg, strResourceID, strProfileID As String
	Dim blnIsDevice As Boolean
	...
	Sub Page_PreRender
		If blnACRead Then		
			...
			Select strPageState
				...
				Case "EDIT"
					strDBQuery = "SELECT r.resource_id, r.name, r.type, r.category, r.description, r.date_created, r.date_modified, p.profile_id FROM Resources r LEFT JOIN DeviceProfiles p ON r.resource_id = p.resource_id WHERE r.resource_id = @ResourceID;"
					cmdDBQuery = New SqlCommand(strDBQuery, conWebLabLS)
					...
					If dtrDBQuery.Read() Then
						Select dtrDBQuery("type")
							Case "DEVICE"
								blnIsDevice = True
								strProfileID = dtrDBQuery("profile_id")
							Case "FUNCTION"
								blnIsDevice = False
					...
					If blnIsDevice Then
						rptDevicePerms.DataSource = cmdDBQuery.ExecuteReader()
						rptDevicePerms.DataBind()
					Else
						rptNonDevPerms.DataSource = cmdDBQuery.ExecuteReader()
						rptNonDevPerms.DataBind()
					End If
	...
	Sub UpdateResource_Command(s As Object, e As CommandEventArgs)
		If blnACGrant Then
			...
			If Session("PermListViewPref") = "limit" Then
				...
				If e.CommandName = "DEVICE" Then
					For loopIdx = 0 To Cint(Request.Form("Gct")) - 1 'CInt(lblGroupCT.Text) - 1
						intCtrlNum = (loopIdx * 2) + 1
						
						lblMappingIDRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":lblMappingID")
						chkCanReadRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanRead")
						chkCanEditRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanEdit")
						chkCanDeleteRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanDelete")
						chkCanGrantRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanGrant")
						txtPriorityRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":txtPriority")
				...
				Else
					For loopIdx = 0 To Cint(Request.Form("Gct")) - 1 'CInt(lblGroupCT.Text) - 1
						...
						lblMappingIDRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":lblMappingID")
						chkCanReadRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanRead")
						chkCanEditRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanEdit")
						chkCanDeleteRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanDelete")
						chkCanGrantRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanGrant")
			...
			ElseIf Session("PermListViewPref") = "all" Then
				...
				If e.CommandName = "DEVICE" Then
					For loopIdx = 0 To Cint(Request.Form("Gct")) - 1 'CInt(lblGroupCT.Text) - 1
						...
						lblMappingIDRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":lblMappingID")
						lblClassIDRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":lblClassID")
						chkCanReadRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanRead")
						chkCanEditRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanEdit")
						chkCanDeleteRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanDelete")
						chkCanGrantRef = FindControl("rptDevicePerms:_ctl" & intCtrlNum & ":chkCanGrant")
				...
				'non-device case
					For loopIdx = 0 To Cint(Request.Form("Gct")) - 1 'CInt(lblGroupCT.Text) - 1
						...
						lblMappingIDRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":lblMappingID")
						lblClassIDRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":lblClassID")
						chkCanReadRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanRead")
						chkCanEditRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanEdit")
						chkCanDeleteRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanDelete")
						chkCanGrantRef = FindControl("rptNonDevPerms:_ctl" & intCtrlNum & ":chkCanGrant")
						...
						<table border=0 cellpadding=3 cellspacing=0 width=100%>
							<tr bgcolor="#e0e0e0">
								<td align=left>
									...
									<%If blnIsDevice And blnDMRead Then%>
									<font class="extra-small">(<a href="/admin/device-profiles.aspx?pid=<%=strProfileID%>" target=main>View Device Setup</a>)</font>
									<%End If%>
								</td>
							</tr>
							<tr>
											...
											<td colspan=2>
												<font class="regular">
													...
													<%If blnIsDevice Then%>
														...
														<asp:Repeater
															...
															<ItemTemplate>
											 						<tr>
											 							<td>
											 								<font class="regular">
											 									&nbsp;<a href="/admin/usage-classes.aspx?cid=<%#Container.DataItem("class_id")%>" target=main><%#Container.DataItem("name")%></a>
											 									<asp:Label
											 										ID="lblMappingID"
											 										Visible="False"
														...
														<input type=hidden name=Gct value=<%=rptDevicePerms.Items.Count()%>
													<%Else%>
														...
														<input type=hidden name=Gct value=<%=rptNonDevPerms.Items.Count()%>
													<%End If%>
													&nbsp;
													<asp:Label 
														ID="lblErrorMsg"
											...
											<tr>
												<td colspan=2>
													<font class="regular">
														...
														<%If blnDMEdit Then%>
															Device resources can be added via the <a href="/admin/device-profiles.aspx?mode=add" target=main>Create Device Profile form</a>.
														<%End If%>
														<br>&nbsp;
													</font>
